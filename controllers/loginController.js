...
//         const { fullName, email, password, role, space_name, space_id } = req.body;
//         const response = await registerUser(fullName, email, password, role, space_name, space_id);
...
const { signInUser, registerUser, getUsers } = require("./../services/users.service.js");
const { homeConnectAuth, homeConnectToken } = require("./../api/homeConnect.js");
const { checkforUserDistance } = require("./../api/location.js");
const _ = require("lodash");
...
exports.loginControllers = {
  ...
  // --------------------------------- User Roles ---------------------------------
  async getUserRole(req, res) {
    res.json({ role: req.user.role });  // Assumes role is added to req.user by middleware
  },
